...
def euler_from_quaternion(x,y,z,w): #checked
    t0 = +2.0 * (w * x + y * z)
    t1 = +1.0 - 2.0 * (x * x + y * y)
...
def bspline_planning(x, y, sn): #checked
    N = 2
    t = range(len(x))
...
def pure_pursuit(current_x, current_y, current_heading, path,index): #checked 
    ...
    if desired_steering_angle > math.pi:#
        desired_steering_angle -= 2 * math.pi #Ensure the desired_steering_angle is within the range −𝜋 to 𝜋 by adjusting it if it's outside this range.
...
def costmap(data,width,height,resolution): #checked 
...
def bspline_planning(array, sn): #checked 
    try:
